...
export function MedicineAddDialog({ isOpen, onClose, onAdd }: MedicineAddDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const newMedicine = await MedicineService.createMedicine({
        name: formData.name,
      ...
      setErrors({ submit: error instanceof Error ? error.message : "Failed to create medicine" })
    } finally {
      setIsLoading(false)
    }
  }
          ...
          <div>
            ...
            <div className="relative mt-2">
              <Input
                ...
                className={`${errors.expiryDate ? "border-red-500" : ""} pr-10`}
